...
async function main() {
  ...
  await eth.giveToken(ethUserWallet.address, 'aUSDC', BigInt(1000e6));
  ...
  console.log((await usdcEthContract.balanceOf(ethUserWallet.address)) / 1e6, 'aUSDC in Ethereum wallet');
  console.log((await usdcAvalancheContract.balanceOf(avalancheUserWallet.address)) / 1e6, 'aUSDC in Avalanche wallet');
}
